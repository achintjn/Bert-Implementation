...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    # print(head_size)
    #print("0")
    #torch.Size([8, 12, 39, 64])
    #q_s = query.contiguous().view(batch_size, -1, num_attention_heads,head_size).transpose(1,2)  # q_s: [batch_size x n_heads x len_q x d_k]
    #print(q_s.size())
    #k_s = key.contiguous().view(batch_size, -1, num_attention_heads,head_size).transpose(1,2)  # k_s: [batch_size x n_heads x len_k x d_k]
    #v_s = value.contiguous().view(batch_size, -1, num_attention_heads,head_size).transpose(1,2)  # v_s: [batch_size x n_heads x len_k x d_v]
    # print("1")
    scores = torch.matmul(query, key.transpose(-1, -2)) /  math.sqrt(head_size)
    # print("2")
    # print(attention_mask.size())
    #scores.masked_fill(attention_mask,-1e9)
    ...
    #context = context.transpose(1, 2).contiguous().view(batch_size, -1, num_attention_heads * seq_len)
    # next, we need to concat multi-heads and recover the original shape [bs, seq_len, num_attention_heads * attention_head_size = hidden_size]
    context = context.transpose(1, 2).contiguous().view(batch_size, -1, num_attention_heads * head_size)
    # print("final size",context.size())
    #output = nn.Linear(self.all_head_size, self.all_head_size)(context)#[bs, seq_len, hidden_state]
...
class BertLayer(nn.Module):
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    # add-norm layer
    norm_out = self.add_norm(hidden_states, attention_out, self.attention_dense, self.attention_dropout, self.attention_layer_norm)

    # feed forward
    feed_forward = self.interm_af(self.interm_dense(norm_out))

    # another add-norm layer
    norm_out = self.add_norm(norm_out, feed_forward, self.out_dense, self.out_dropout, self.out_layer_norm)
